# Unpluged-Al

Materjalid õpetajate ja tehisintellekti teemale — eriti **KOV tasandi** otsuste ja toe jaoks.

Riik viib ellu TI-hüppe; õpetajate igapäevane olukord sõltub paljuski **kohalikust omavalitsusest** (munitsipaalkoolid, aeg, tugi, töökorraldus).

## DOCX allalaadimised

**[Unpluged-Al KOV pakett (DOCX)](https://github.com/reneealuste-commits/Unpluged-Al/raw/main/docs/docx/Unpluged-Al-KOV-pakett.docx)** — kõik peamised materjalid ühes failis.

Täielik nimekiri: [docs/ALLALAADIMISED.md](docs/ALLALAADIMISED.md)

## Dokumentid

| Fail | Sisu |
|------|------|
| [docs/kov-strateegia.md](docs/kov-strateegia.md) | 12 kuu rada: kelle juurde minna, mida jagada |
| [docs/kov-luhiteave.md](docs/kov-luhiteave.md) | Üheleheline lühiteave volikogule / ametnikele |
| [docs/kohtumise-agenda.md](docs/kohtumise-agenda.md) | 60–90 min kohtumise mall |
| [docs/peegli-metoodika.md](docs/peegli-metoodika.md) | Peegli harjutus enne TI-otsuseid |
| [docs/ekiri-harju-kov.md](docs/ekiri-harju-kov.md) | E-kirja mall Harju KOV haridusspetsialistidele |
| [docs/1-1-algus-plan.md](docs/1-1-algus-plan.md) | 1-1 rada: alusta kaugemalt, üks KOV korraga |
| [docs/ekiri-1-1-kuusalu.md](docs/ekiri-1-1-kuusalu.md) | **Esimene** 1-1 e-kiri (Kuusalu) |

## Peegel

- Tartu Ülikooli ajakiri [Peegel](https://peegel.ut.ee/index.php/node/1003) dokumenteeris õpetajate häda TI kasutamisel.
- **Peegli metoodika** = aus peegeldamine enne suuri muudatusi; raamat töötab nagu peegel — näitab, mida organisatsioon tegelikult vajab.

## Kontakt

Renee Aluste — reneealuste@gmail.com
