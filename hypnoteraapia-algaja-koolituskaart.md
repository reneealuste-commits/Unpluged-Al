# KOOLITUSKAART HY1 — Hüpnoosi algaja juhised

**Unpluged-Al** | Kodune juhend | **Kestus: 15–25 min**

| | |
|---|---|
| **Kood** | HY1 |
| **Teema** | Turvaline hüpnoos — rahunemine, uni, fookus |
| **Versioon** | 1.2 · 16. august 2026 |
| **Kasutaja** | Iga mees — juhib kaaslasele |
| **Sihtgrupp** | Mees juhib kaaslasele (abikaasa, sõber, meeskonnakaaslane) |
| **Eeldused** | **Mitte** trauma, PTSD, kriis, psühhootiline episood — siis spetsialist |
| **Seotud** | `hypnoteraapia-test-nr1-kaaslasega.md` · `hypnoteraapia-algaja-juhend.md` · `Hypnoteraapia-HY1.docx` |

> *See kaart on hariduslik. Ei asenda hünoterapeudi ega psühholoogi abi.*

---

## KOOLITUSKAART (prindi see leht — taskus või külmkapis)

```
╔══════════════════════════════════════════════════════════════════╗
║  HY1 — HÜPNOOSI ALGAJA JUHISED                   15–25 min       ║
║  Unpluged-Al · iga mees · kaaslasele juhtimine                  ║
╠══════════════════════════════════════════════════════════════════╣
║  OLULINE — loe enne alustamist                                   ║
║  Kerge stress, uni, fookus — MITTE trauma/PTSD/kriis.            ║
║  Sa SAAD igal hetkel silmad avada ja lõpetada.                   ║
║  STOP kohe kui distress > 8/10 või “õhust väljas”.               ║
╠══════════════════════════════════════════════════════════════════╣
║  VAHENDID          │  EESMÄRK (vali ÜKS)                         ║
║  vaikne koht, vesi │  □ Rahunemine  □ Uni  □ Fookus enne tegevust║
║  taimer 15–25 min  │  □ Debriefi ettevalmistus (5 min variant)   ║
║  paber stressile   │  Kirjuta: _________________________________   ║
╠══════════════════════════════════════════════════════════════════╣
║  PROTSESS (kiire viide)                                          ║
║  Ankur 3× hingetõmme → Keha skaneering → Turvakoht →            ║
║  Suggestioon 8–12 min → Tagasi 5-1 → Vesi → “Mis muutus?”        ║
╠══════════════════════════════════════════════════════════════════╣
║  SUGGESTIOON (loe vaikselt või kuula)                            ║
║  „Ma rahunen loomulikult. Mu keha teab, kuidas tulla rahule.     ║
║   Ma olen turvaliselt. Iga hingetõmme toob rohkem kergust.”      ║
╠══════════════════════════════════════════════════════════════════╣
║  STOP KORRAL                                                     ║
║  Ava silmad. Jalad põrandale. 5-4-3-2-1 maandamine. Vesi.         ║
║  Kõnni. Pöördu spetsialisti poole kui vaja.                      ║
╠══════════════════════════════════════════════════════════════════╣
║  KEELATUD                    │  KOHUSTUSLIK                      ║
║  Trauma avamine ilma oskuseta │ Üks eesmärk korraga               ║
║  “Sa ei mäleta” / sund         │ Nõusolek enne alustamist          ║
║  Diagnoosimine                 │ 1 lause integratsioon/debrief     ║
╚══════════════════════════════════════════════════════════════════╝
```

---

## PROTSESS — täielik sisu (tagakülg)

| Samm | Faas | Mida teed | Aeg |
|------|------|-----------|-----|
| **0** | **Valmistumine** | Vaikne koht. Vesi. Üks eesmärk kirja. Stress 0–10. | 2 min |
| **1** | **Ankur** | Istu/lama. Jalad maas. Hingamine: sisse 4, välja 6 × 3. | 2 min |
| **2** | **Keha skaneering** | Tähelepanu peast jalgadeni. Igas kohas: “Lõdvestu.” | 3 min |
| **3** | **Turvakoht** | Kujutle turvalist kohta. Mida näed, kuuled, tunned? | 2 min |
| **4** | **Suggestioon** | Too eesmärk meelde. Korda HY1 lauset. Hinga aeglaselt. | 8–12 min |
| **5** | **Tagasitulek** | Loenda 5 → 1. Liiguta sõrmi. Ava silmad. Joo vett. | 2 min |
| **6** | **Integratsioon** | Kirjuta: “Mis muutus?” Stress 0–10 uuesti. | 1 min |

---

## MEETODID — vali stiil

| Stiil | Kuidas | Millal |
|-------|--------|--------|
| **A. Ise** | Loed skripti vaikselt või mõtled | kodus, öösel |
| **B. Salvestis** | Kuulad oma või usaldusväärset häält | uni, korduv rituaal |
| **C. Paariline** | Teine loeb aeglaselt, sina kuulad | esimene kord, tugi |
| **D. Kiir (5 min)** | Ainult ankur + 3 suggestiooni + tagasi | enne debriefi/KOV |

---

## PAARILINE PROTOKOLL

| Samm | Juht (B) | Osaleja (A) |
|------|----------|-------------|
| 1 | “Sa saad igal hetkel lõpetada.” | Kinnitab |
| 2 | Loeb sammud 1–3 | Järgib |
| 3 | Loeb suggestioone 8–10 min | Kuulab, hingab |
| 4 | “Loendame 5–1. Ava silmad.” | Tagasi |
| 5 | “Mis muutus? Stress 0–10?” | Vastab |

### Mida juht EI tee

| ❌ | ✓ Selle asemel |
|----|----------------|
| Sundib silmi kinni hoidma | “Ava silmad, kui tahad” |
| Avab raske mineviku | Hoia eesmärk kerge |
| Diagnoosib | “Kuidas su keha tunneb?” |
| “Sa oled sügaval hüpnoosis” | “Sa oled rahulikult fokusseeritud” |

---

## ENESEREFLEKSIOON (ei ole eksam)

| Küsimus | ☐ jah ☐ osaliselt ☐ ei |
|---------|------------------------|
| Sain valida ühe selge eesmärgi? | |
| Tundsin, et saan igal hetkel lõpetada? | |
| Stress/distress langes või stabiliseerus? | |
| Kirjutasin “mis muutus?” | |

**Märge (valikuline):** Kuupäev ______ Eesmärk ______ Stress enne ___ pärast ___

---

## SEOTUD

| Fail | Sisu |
|------|------|
| `hypnoteraapia-test-nr1-kaaslasega.md` | Formaalne test (Automaadi test vorming) |
| `hypnoteraapia-algaja-juhend.md` | Täielik juhend + SWOT |
| `hypnoteraapia-algaja-plaankonspekt.md` | 90 min grupikoolitus |
| `Hypnoteraapia-HY1.docx` | Word (juhend + kaart + plaankonspekt) |
| `debrief-kaart-malevapealik.pdf` | Debrief pärast olulist tegevust |

---

*HY1 v1.2 · Unpluged-Al · prinditav koolituskaart · iga mees kaaslasele*
