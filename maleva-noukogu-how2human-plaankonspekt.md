# PLAANKONSPEKT | MALEVA NÕUKOGU × HOW2HUMAN

## Juhtide ruum — lahendused, mitte loengud

| | |
|---|---|
| **Bränd** | **Maleva Nõukogu** (üritus) · **How2Human** (inimlik suhtlus) |
| **Alapealkiri** | *Mitte seminar. Mitte retreat. Mitte grupp.* |
| **Missioon** | Luua Eestis **ruum**, kus tippjuht saab **granulaarseid lahendusi** — mitte slides, vaid praktikat |
| **Sihtgrupp** | Juht, kelle otsused mõjutavad **miljoneid** (€, inimesi või mõlemat) |
| **Asukoht** | Hobuserantso (ratsutamine + vaiksed „vahehetked“) |
| **Formaat** | 12–20 kohta · 2 päeva · 1-on-1 + debrief |
| **Koostaja** | Renee Aluste |
| **Versioon** | 1.0 · 21. august 2026 |
| **Seotud** | `juhtimisvideo-riho-remo-oppejuhend.md` · `debrief-kaart-malevapealik.pdf` · `olessanded-alluksustele.md` · EF Council ([video](https://youtu.be/tU_Dl0Ic3ao)) |

---

## 1. MIS SEE ON — ÜHE LAUSEGA

> **Maleva Nõukogu** on piiratud kohtadega **juhtide ruum**, kus neli kihti — **rats, suhtlus, How2Human, debrief** — annavad tippjuhile praktilised lahendused unikaalsetele väljakutsetele.

See **ei ole**:
- ❌ konverents ega „juhtimispäev“
- ❌ motivatsiooniseminar
- ❌ meeskonna building laager

See **on**:
- ✅ **Nõukogu** — otsused ja lahendused, mitte teooria
- ✅ **Ruum** — füüsiline ja psühholoogiline turvalisus ausaks vestluseks
- ✅ **Precision** — ambu/vibu, kehakeel, hobune — kõik õpetavad **täpsust**

---

## 2. KELLELE (EESTI TURG)

| Segment | Näide | Miks tuleb |
|---------|-------|------------|
| **Ettevõtte omanik / CEO** | 50–500 töötajat, €5M+ käive | Meeskond ei võta vastutust; otsused venivad |
| **KOV juht** | Linnapea, volikogu esimees | Poliitiline vaikimine; ei kuula maastikku |
| **Malevapealik / KL juht** | 15 maleva võrgustik | Tsiviil-maastiku side nõrk; debrief puudub |
| **Haridusjuht** | Direktor, komisjoni esimees | Organisatsioon premeerib keskmisust |
| **Investor / nõukogu liige** | Portfell, mitu ettevõtet | Vajab teistsugust nõu kui advokaat või audiitor |

**Kvalifikatsioon (self-select):**
- Otsused mõjutavad **>1000 inimest** või **>€1M** aastas
- „Mul on juba edu — aga ma vajan **rohkem**“ (EF Council loogika)
- Valmis **tunnistama esimesena**, mitte süüdistama

**Ei ole sihtgrupp:** algajad juhid, massiline avatud koolitus, poliitiline kampaania.

---

## 3. BRÄNDI ARHITEKTUUR

```
                    MALEVA NÕUKOGU
                   (juhtide ruum · 2 päeva)
                           │
        ┌──────────────────┼──────────────────┐
        │                  │                  │
    RATS              SUHTLUS           HOW2HUMAN
  (hobune)         (juht ↔ meeskond)   (inimene ↔ inimene)
        │                  │                  │
        └──────────────────┴──────────────────┘
                           │
                      DEBRIEF
              (lahendused · 1-on-1 · protokoll)
```

| Kiht | Nimi | Mida annab |
|------|------|------------|
| **1** | **Rats Juhtimine** | Usaldus, fookus, juht ilma sundimata — hobune peegeldab juhti |
| **2** | **Suhtlus Juht** | Aus side meeskonnaga; debrief; „mida ma tegelikult ütlesin?“ |
| **3** | **How2Human** | Kehakeel, peegeldamine, kuulamine — **kaks inimest**, üks ruum |
| **4** | **Debrief & Lahendus** | Extreme Ownership + `debrief-kaart-malevapealik.pdf` protokoll |

---

## 4. HOW2HUMAN — METOODIKA

**How2Human** = *How to (be) human* + *two humans in the room*.

Kaks inimest. Üks ruum. Null slaidi.

### 4.1 Kolm oskust

| Oskus | Mida õpid | Praktiline tulemus |
|-------|-----------|-------------------|
| **Kehakeele lugemine** | Mikroilmid, postuur, tempo, vaikus | Sa näed, **mis tegelikult toimub**, enne kui keegi seda ütleb |
| **Peegeldamine (mirroring)** | Keha, tempo, sõnavalik (subtle) | Rapport ilma manipulatsioonita — teine tunneb, et teda **nähtakse** |
| **Peegeldamine (reflecting)** | „Ma kuulen, et…“ + emotsiooni nimetamine | Konflikt → selgus; alluv julgeb rääkida |

### 4.2 How2Human harjutused (rantso)

| # | Harjutus | Kestus | Sisu |
|---|----------|--------|------|
| H1 | **Kaks tooli** | 20 min | A ja B — üks räägib, teine ainult peegeldab (reflect). Vahetus. Debrief. |
| H2 | **Keha skaneering partneriga** | 15 min | Enne rasket vestlust — mis kehas toimub? (pinge, kiirustamine) |
| H3 | **Vaikuse 90 sek** | 10 min | Kaks juhti. 90 s vaikus. Mida märkasid teises? |
| H4 | **Rats peegeldab** | 45 min | Hobune reageerib kohe — juht näeb oma keha-keha suhet **reaalajas** |
| H5 | **1-on-1 nõu** | 30 min | Mentor + osaleja. How2Human protokoll. Konfidentsiaalne. |

### 4.3 How2Human reeglid

1. **Konfidentsiaalsus** — mis ruumis, jääb ruumi (v.a. oht)
2. **Juht räägib esimesena ausalt** — siis teised julgevad
3. **Peegeldamine ≠ kopeerimine** — subtle, mitte karikatuur
4. **Keha enne sõna** — enne vastamist 3 sekundit: „mis mu kehas toimub?“

---

## 5. RATS JUHTIMINE

Hobune on **peegel**, mida slaid ei anna.

| Õppetund | Hobune | Juhtimine |
|----------|--------|-----------|
| Usaldus | Ei liigu, kui sa pinges | Meeskond ei liigu, kui sa hirmus |
| Selgus | Selge signaal = selge reaktsioon | Segane juht = segane meeskond |
| Tempo | Kiirustamine = vastupanu | Otsused „yesterday“ = tagasilöök |
| Ausus | Hobune ei valeta | Meeskond teab, kui sa valetsid |

**Formaat:** 2–3h · max 8 ratsutajat korraga · instruktor + debrief iga ringi järel.

**„Little moments“** (EF Council): vestlus hobuse juures, sõit metsateel, jutt tallis — sealt sündivad suhted, mitte plenaarilt.

---

## 6. SUHTLUS JUHT — DEBRIEF KESKNE

Põhineb Remo Ojaste / Extreme Ownership loogikal (`juhtimisvideo-riho-remo-oppejuhend.md`):

| Teema | Küsimus debriefis |
|-------|-------------------|
| Vastutus | „Mida **mina** tegin valesti?“ |
| Ego | „Kus ma kaitse ennast, mitte missiooni?“ |
| Info | „Mida ma ei tahtnud kuulda?“ |
| Tegevus | „Mis on **üks** samm homme?“ |

**Tööriist:** `debrief-kaart-malevapealik.pdf` — igale osalejale kaasa.

---

## 7. PROGRAMM — 2 PÄEVA (PILOOT)

### Päev 1 — „Ruum avaneb“

| Aeg | Plokk | Sisu |
|-----|-------|------|
| 09:00 | Saabumine | Kohv. Ilma slaidideta tutvustus: *„See ei ole seminar.“* |
| 09:30 | **Avamine** | Miks sa siin oled? Mis on sinu **unique challenge**? (max 2 min inimese kohta) |
| 10:30 | **Rats Juhtimine I** | Turvalisus, hobune, esimene kontakt |
| 12:30 | Lõuna | Ühine laud — vestlus, mitte programm |
| 14:00 | **How2Human H1–H3** | Kaks tooli · vaikus · kehakeel |
| 16:00 | **Precision** | Ambu/vibu (valik) + debrief iga seeria järel |
| 18:00 | Vaba | Rantso, sauna, „vahehetked“ |
| 20:00 | **Tule ring** | 1-on-1 vestlused · konfidentsiaalsus |

### Päev 2 — „Lahendused“

| Aeg | Plokk | Sisu |
|-----|-------|------|
| 08:00 | Hommik | Kerge liikumine / rats (vabatahtlik) |
| 09:30 | **Suhtlus Juht** | Extreme Ownership tuum · Dixon / Ühtegi kontrast |
| 11:00 | **Rats Juhtimine II** | Grupiülesanne hobustega · debrief |
| 12:30 | Lõuna | |
| 14:00 | **How2Human H5** | 1-on-1 mentorlus (30 min × osaleja) |
| 16:00 | **Lahenduste kaart** | Iga osaleja: 1 väljakutse · 1 lahendus · 1 samm 72h jooksul |
| 17:00 | **Lõpetus** | Mitte „tänan, et tulite“ — *„Mida sa varastad ja rakendad?“* |

---

## 8. POSITSIONEERIMINE EESTI TURUL

| | EF Council (USA) | Maleva Nõukogu (EE) |
|---|---|---|
| Keel | Inglise | **Eesti** (+ vajadusel EN grupp) |
| Asukoht | Montana ranch | **Eesti hobuserantso** |
| Precision | Tulirelv | **Ambu/vibu** + How2Human |
| Kultuur | Echelon Front | **Ühtegi + Remo + Malev** — kohalik identiteet |
| Film/DNA | — | **Malev (2005)** — kärajad, Uru, kokku kutsumine |
| Hind | $$$$$ | Premium, aga **Eesti reaalsus** (vt all) |

**Erinevus Combat Ready-st:** CR = koolitus ja metoodika. **Maleva Nõukogu** = **ruum + lahendused + suhted** — CR võib olla partner (intro enne nõukogu).

---

## 9. ÄRIMUDEL (ESIALGNE)

| Element | Piloot | Skaleeritud |
|---------|--------|-------------|
| Kohti | 12–15 | 15–20 |
| Sagedus | 2× aastas | 4–6× aastas |
| Hind (osaleja) | €2 500–4 500 | €3 500–6 000 |
| Kaasatud | 1 peamene mentor + rats + How2Human | + külaline (Remo-tüüp / Ühtegi) |
| Tulu (15 × €3 500) | ~€52 500 / üritus | Ekspertiis + ruum + kasum |

**Miks maksavad:** mitte tunnid, vaid **juurdepääs ruumile, inimestele ja lahendustele**, mida büroos ei saa.

---

## 10. TURUNDUS — KOLM LAUSE

**Pealkiri:** Maleva Nõukogu  

**Alapealkiri:** *Mitte seminar. Mitte retreat. Mitte grupp.*  

**Keha:**
> Kui sinu otsused mõjutavad miljoneid — vajad rohkem kui slaidid. Hobune. How2Human. Debrief. **12 kohta.**

**CTA:** *„Kas see on minu jaoks?“* → lühike kvalifikatsioonivorm (5 küsimust), mitte avalik piletimüük.

---

## 11. KVALIFIKATSIOONIVORM (5 KÜSIMUST)

1. Mis on sinu roll ja kui palju inimesi/raha sinu otsused mõjutavad?
2. Mis on **üks** väljakutse, mida sa pole suutnud lahendada büroos?
3. Kas oled valmis **tunnistama esimesena**, kui midagi ei tööta?
4. Miks sa vajad **rohkem** kui tavalist juhtimiskoolitust?
5. Kes soovitab sind? (valikuline — premium filter)

---

## 12. PARTNERID JA ÖKOSÜSTEEM

```
Combat Ready intro (2h)  →  Maleva Nõukogu (2p)  →  Igakuine debrief (malev/tsiviil)
         │                        │                           │
    avab ukse              transformeerib               hoiab aususe
```

| Partner | Roll |
|---------|------|
| **Combat Ready / Remo** | Extreme Ownership intro enne nõukogu (valikuline) |
| **Kaitseliit / malev** | Malevapealikud + maastiku side |
| **Rantso** | Asukoht, rats, majutus |
| **How2Human instruktor** | Kehakeel, peegeldamine |
| **Ambu/vibu instruktor** | Precision + ohutus |

---

## 13. OHUTUS JA ÕIGUS

| Tegevus | Nõue |
|---------|------|
| Ratsutamine | Kiiver, kogenud instruktor, kindlustus |
| Ambu/vibu | Ohutuskoolitus, 3D sihtmärgid, written brief |
| How2Human | Konfidentsiaalsus leping · psühholoogiline turvalisus |
| Üritus | Osaleja leping · foto/video reeglid |

---

## 14. JÄRGMISED SAMMUD

| # | Samm | Tähtaeg |
|---|------|---------|
| 1 | Kinnita bränd: **Maleva Nõukogu** + **How2Human** | Kohe |
| 2 | Rantso partner + rats + ambu/vibu instruktor | 4 nädalat |
| 3 | Piloot **12 kohta** — 1 malev + 5 tsiviiljuhti + 6 ettevõtte | Q1 2027 |
| 4 | Landing (1 leht) + kvalifikatsioonivorm | 6 nädalat |
| 5 | EF Council video stiilis **60s reklaam** (eesti keeles) | 8 nädalat |
| 6 | Debrief #1 — skaleeri või korrigeeri | +2 nädalat pärast pilooti |

---

## 15. LÕPETUSE Lause (nagu EF videol)

> *See ei ole grupp. See ei ole seminar. See ei ole laager.*  
> ***See on Maleva Nõukogu — ruum, kus juht saab lahendusi.***

---

*Seotud: `OPORD-sise.md` · `debrief-kaart-malevapealik.pdf` · `juhtimisvideo-riho-remo-oppejuhend.md` · [EF Council video](https://youtu.be/tU_Dl0Ic3ao)*
