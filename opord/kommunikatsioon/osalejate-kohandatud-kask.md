# Osalejate kohandatud käsk

**Eesmärk:** Iga osaleja saab oma rolli, eesmärgi ja käitumisjuhise.  
**Seotud:** Lisa K, Lisa Q, Lisa AJ, Lisa AQ  
**Kuupäev:** 24. juuli 2026  
**Koordinaator:** Renee Aluste

> **Reegel:** Isiklik e-kiri, mitte masspost. Iga saaja = isiklik link (Lisa AJ).  
> **Kuldne taganemistee:** „Tänan, palun eemalda minu nimi.“ — ilma surveta.

---

## Ülevaade

| Kategooria | Arv |
|------------|-----|
| Kohandatud käsk valmis | 16 |
| Email CSV-s kokku | vaata `osalejate-emailid.csv` |
| Email puudub (vaja otsida) | 12 |

---

## Podcast ja meedia (eesti keel primaarselt)

> **Reegel:** `kommunikatsioon/soovitus-meedia-eesti-primaar.md`

**PERE / KRIIS** → [Papsid.ee Podcast](https://papsid.ee/podcast/) (ET)  
**Muu** → Ava Oma Silmad & Ajuloputus S4 #10 — „Maatriksi Lapsed“ (ET)

| Papsid.ee (ET) | [https://papsid.ee/podcast/](https://papsid.ee/podcast/) · [Apple EE](https://podcasts.apple.com/ee/podcast/papsid-ee-podcast/id1768003452) |
| Ava Oma Silmad (ET) | [https://shows.acast.com/avaomasilmad/episodes/ee-jata-navigatsioon-vahele-loomine-avataripilt-ava-oma-silm](https://shows.acast.com/avaomasilmad/episodes/ee-jata-navigatsioon-vahele-loomine-avataripilt-ava-oma-silm) · [Apple EE](https://podcasts.apple.com/ee/podcast/ava-oma-silmad-podcast/id1541890084) |

---

## 1. Tanel Jäppinen

| | |
|---|---|
| **E-post** | `` |
| **Organisatsioon** | Parenting Solutions / Laste Supermängud |
| **Roll** | Noorte- ja peretasandi juht; Laste Supermängud |
| **Eesmärk** | Noorte praktiline vastutus enne algoritmi |
| **Sidepakk** | P1-C-PERE + P1-A-KRIIS (suunamine) |
| **Kanal** | E-kiri / kohtumine |

### Sinu ülesanded

1. Integreeri PEEGEL_TEE_C (pere) noorte programmidesse kui sobib
2. Vanemate mentorlus: üks tegu päevas, mitte loeng
3. Suuna kriisis olevad isad Lisa H-le (PEEGEL_TEE_A)
4. Anna tagasiside: mis töötab 7–12-aastastega

### Käitumisjuhis

Mänguline, soe. Lapsed = turvalisus enne sisu. Ära suru.

### E-kirja avamine (mustand)

> Tere Tanel,
>
> Jagame sulle Operatsioon „Peegel“ materjali — mitte käsk, vaid paranemis-teekond. Sinu roll: **Noorte- ja peretasandi juht**.
>
> Isiklik link: [PEEGEL_TUUM / vastav tee PDF]
>
> **Valikuline kuulamine:** Papsid.ee Podcast  
> Isa kriisis — Lisa H kontekstis  
> Kuula: [https://papsid.ee/podcast/](https://papsid.ee/podcast/) · [Apple EE](https://podcasts.apple.com/ee/podcast/papsid-ee-podcast/id1768003452)  
> Täpsem: `kommunikatsioon/soovitus-papsid-podcast.md`
>
> Kui soovid mitte osaleda — üks lause piisab. Austan seda.
>
> Renee Aluste

---

## 2. Marge Sillaste

| | |
|---|---|
| **E-post** | `margemargarethe@gmail.com` |
| **Organisatsioon** | Supervisioon / tervishoid |
| **Roll** | Juhtide supervisioon ja coaching |
| **Eesmärk** | Inimeste juhtimine, mitte protsesside kontroll |
| **Sidepakk** | P1-F-JUHT |
| **Kanal** | E-kiri privaatne |

### Sinu ülesanded

1. Loe Lisa I (Steiger) ja Lisa P (trauma) enne juhtidega tööd
2. Peegelda juhtidele: turvalisus enne tulemust
3. Anna tagasiside Reneele supervisiooni vajaduste kohta
4. Kasuta kuldset taganemisteed kui keegi soovib nime eemaldada

### Käitumisjuhis

Rahulik FM-DJ. Kuula ilma parandamiseta. Konfidentsiaalsus.

### E-kirja avamine (mustand)

> Tere Marge,
>
> Jagame sulle Operatsioon „Peegel“ materjali — mitte käsk, vaid paranemis-teekond. Sinu roll: **Juhtide supervisioon ja coaching**.
>
> Isiklik link: [PEEGEL_TUUM / vastav tee PDF]
>
> **Valikuline kuulamine (~1h 20 min):** Ava Oma Silmad & Ajuloputus S4 #10 — „Maatriksi Lapsed“  
> Juhi vaatenurgast: kaitse enne tulemust. Kui keegi on ellujäämisrežiimis, ta ei näe plaani (Lisa AT).  
> Kuula: [https://shows.acast.com/avaomasilmad/episodes/ee-jata-navigatsioon-vahele-loomine-avataripilt-ava-oma-silm](https://shows.acast.com/avaomasilmad/episodes/ee-jata-navigatsioon-vahele-loomine-avataripilt-ava-oma-silm) · [katrinlucas.com](https://katrinlucas.com/podcast/) · [Apple EE](https://podcasts.apple.com/ee/podcast/ava-oma-silmad-podcast/id1541890084) · [YouTube](https://www.youtube.com/@AJULOPUTUS)  
> Täpsem kokkuvõte: `kommunikatsioon/soovitus-maatriksi-lapsed.md`
>
> Kui soovid mitte osaleda — üks lause piisab. Austan seda.
>
> Renee Aluste

---

## 3. Ott Pärna

| | |
|---|---|
| **E-post** | `info@techno.ee` |
| **Organisatsioon** | Techno TLN |
| **Roll** | Direktor; haridustaseme strateegiline partner |
| **Eesmärk** | Kutse- ja tehnoloogiaharidus noorte vastupanuvõimes |
| **Sidepakk** | P1-F-JUHT |
| **Kanal** | Ametlik e-kiri |

### Sinu ülesanded

1. Tutvu PEEGEL_TEE_F (juht/koolitus) — isiklik link
2. Hinda: kas Steigeri põhimõtted sobivad õppekavaga (Lisa I)
3. Suuna kriisiteemad koolipsühholoogile, mitte debatile
4. Vastus üks lausega: kas ja kuidas edasi

### Käitumisjuhis

Austav keel (teietamine). Ametlik toon. Ei propaganda.

### E-kirja avamine (mustand)

> Tere Ott,
>
> Jagame sulle Operatsioon „Peegel“ materjali — mitte käsk, vaid paranemis-teekond. Sinu roll: **Direktor**.
>
> Isiklik link: [PEEGEL_TUUM / vastav tee PDF]
>
> **Valikuline kuulamine (~1h 20 min):** Ava Oma Silmad & Ajuloputus S4 #10 — „Maatriksi Lapsed“  
> Juhi vaatenurgast: kaitse enne tulemust. Kui keegi on ellujäämisrežiimis, ta ei näe plaani (Lisa AT).  
> Kuula: [https://shows.acast.com/avaomasilmad/episodes/ee-jata-navigatsioon-vahele-loomine-avataripilt-ava-oma-silm](https://shows.acast.com/avaomasilmad/episodes/ee-jata-navigatsioon-vahele-loomine-avataripilt-ava-oma-silm) · [katrinlucas.com](https://katrinlucas.com/podcast/) · [Apple EE](https://podcasts.apple.com/ee/podcast/ava-oma-silmad-podcast/id1541890084) · [YouTube](https://www.youtube.com/@AJULOPUTUS)  
> Täpsem kokkuvõte: `kommunikatsioon/soovitus-maatriksi-lapsed.md`
>
> Kui soovid mitte osaleda — üks lause piisab. Austan seda.
>
> Renee Aluste

---

## 4. Kristel Bankier

| | |
|---|---|
| **E-post** | `kristel.bankier@techno.ee` |
| **Organisatsioon** | Techno TLN |
| **Roll** | Finants / partnerlus |
| **Eesmärk** | Siduda haridus ja kogukonna partnerid |
| **Sidepakk** | P0-TUUM |
| **Kanal** | E-kiri |

### Sinu ülesanded

1. Loe Lisa AM (realistlik maht) enne partnerluse otsust
2. Hinda koostöövõimalusi ilma ulmeliste numbriteta
3. Suuna tehnilised küsimused Ott Pärnale

### Käitumisjuhis

Professionaalne, faktipõhine. Ära välju kontekstist.

### E-kirja avamine (mustand)

> Tere Kristel,
>
> Jagame sulle Operatsioon „Peegel“ materjali — mitte käsk, vaid paranemis-teekond. Sinu roll: **Finants / partnerlus**.
>
> Isiklik link: [PEEGEL_TUUM / vastav tee PDF]
>
> **Valikuline kuulamine (~1h 20 min):** Ava Oma Silmad & Ajuloputus S4 #10 — „Maatriksi Lapsed“  
> Valikuline teadlikkuse kanal — sobib neile, kes tahavad sügavamat konteksti enne OPORD-i.  
> Kuula: [https://shows.acast.com/avaomasilmad/episodes/ee-jata-navigatsioon-vahele-loomine-avataripilt-ava-oma-silm](https://shows.acast.com/avaomasilmad/episodes/ee-jata-navigatsioon-vahele-loomine-avataripilt-ava-oma-silm) · [katrinlucas.com](https://katrinlucas.com/podcast/) · [Apple EE](https://podcasts.apple.com/ee/podcast/ava-oma-silmad-podcast/id1541890084) · [YouTube](https://www.youtube.com/@AJULOPUTUS)  
> Täpsem kokkuvõte: `kommunikatsioon/soovitus-maatriksi-lapsed.md`
>
> Kui soovid mitte osaleda — üks lause piisab. Austan seda.
>
> Renee Aluste

---

## 5. Kristel Martis

| | |
|---|---|
| **E-post** | `kristel.martis@techno.ee` |
| **Organisatsioon** | Techno TLN |
| **Roll** | Turundus / kogukond |
| **Eesmärk** | Kogukonna sidumine ja aus kommunikatsioon |
| **Sidepakk** | P0-TUUM |
| **Kanal** | E-kiri |

### Sinu ülesanded

1. Tutvu Lisa Q (side SOP) enne mis tahes levitust
2. Mitte masspostitus — ainult opt-in kanalid
3. Soovita P0-TUUM esimene kontakt

### Käitumisjuhis

Mänguline hääl väljas, mitte sarkasm haava peal.

### E-kirja avamine (mustand)

> Tere Kristel,
>
> Jagame sulle Operatsioon „Peegel“ materjali — mitte käsk, vaid paranemis-teekond. Sinu roll: **Turundus / kogukond**.
>
> Isiklik link: [PEEGEL_TUUM / vastav tee PDF]
>
> **Valikuline kuulamine (~1h 20 min):** Ava Oma Silmad & Ajuloputus S4 #10 — „Maatriksi Lapsed“  
> Valikuline teadlikkuse kanal — sobib neile, kes tahavad sügavamat konteksti enne OPORD-i.  
> Kuula: [https://shows.acast.com/avaomasilmad/episodes/ee-jata-navigatsioon-vahele-loomine-avataripilt-ava-oma-silm](https://shows.acast.com/avaomasilmad/episodes/ee-jata-navigatsioon-vahele-loomine-avataripilt-ava-oma-silm) · [katrinlucas.com](https://katrinlucas.com/podcast/) · [Apple EE](https://podcasts.apple.com/ee/podcast/ava-oma-silmad-podcast/id1541890084) · [YouTube](https://www.youtube.com/@AJULOPUTUS)  
> Täpsem kokkuvõte: `kommunikatsioon/soovitus-maatriksi-lapsed.md`
>
> Kui soovid mitte osaleda — üks lause piisab. Austan seda.
>
> Renee Aluste

---

## 6. Johan-Elias Seljamaa

| | |
|---|---|
| **E-post** | `johan-elias.seljamaa@mil.ee` |
| **Organisatsioon** | Kaitseväe Akadeemia |
| **Roll** | Rektor |
| **Eesmärk** | Juhtide areng ja kriitiline mõtlemine |
| **Sidepakk** | P1-F-JUHT |
| **Kanal** | Ametlik e-kiri @mil.ee |

### Sinu ülesanded

1. Loe Lisa I + Lisa P (trauma kriisis)
2. Hinda: kas materjal sobib akadeemilisele kontekstile
3. Suuna Aarne Ermusile akadeemiline tagasiside
4. Kuldne taganemistee kehtib ka ametlikul tasemel

### Käitumisjuhis

Austav, struktureeritud. Lühikesed laused kriisis.

### E-kirja avamine (mustand)

> Tere Johan-Elias,
>
> Jagame sulle Operatsioon „Peegel“ materjali — mitte käsk, vaid paranemis-teekond. Sinu roll: **Rektor**.
>
> Isiklik link: [PEEGEL_TUUM / vastav tee PDF]
>
> **Valikuline kuulamine (~1h 20 min):** Ava Oma Silmad & Ajuloputus S4 #10 — „Maatriksi Lapsed“  
> Juhi vaatenurgast: kaitse enne tulemust. Kui keegi on ellujäämisrežiimis, ta ei näe plaani (Lisa AT).  
> Kuula: [https://shows.acast.com/avaomasilmad/episodes/ee-jata-navigatsioon-vahele-loomine-avataripilt-ava-oma-silm](https://shows.acast.com/avaomasilmad/episodes/ee-jata-navigatsioon-vahele-loomine-avataripilt-ava-oma-silm) · [katrinlucas.com](https://katrinlucas.com/podcast/) · [Apple EE](https://podcasts.apple.com/ee/podcast/ava-oma-silmad-podcast/id1541890084) · [YouTube](https://www.youtube.com/@AJULOPUTUS)  
> Täpsem kokkuvõte: `kommunikatsioon/soovitus-maatriksi-lapsed.md`
>
> Kui soovid mitte osaleda — üks lause piisab. Austan seda.
>
> Renee Aluste

---

## 7. Rainek Kuura

| | |
|---|---|
| **E-post** | `rainek.kuura@mil.ee` |
| **Organisatsioon** | Kaitseväe Akadeemia |
| **Roll** | Õppeosakonna ülem |
| **Eesmärk** | Õppekava ja juhtimiskultuuri sidumine |
| **Sidepakk** | P1-F-JUHT |
| **Kanal** | Ametlik e-kiri |

### Sinu ülesanded

1. Tutvu PEEGEL_TEE_F
2. Anna tagasiside: mis sobiks kadettidele vs juhtidele
3. Trauma-teadlikkus (Lisa P) enne õppetundi

### Käitumisjuhis

Selge, assertiivne kui vaja. Mitte dominants.

### E-kirja avamine (mustand)

> Tere Rainek,
>
> Jagame sulle Operatsioon „Peegel“ materjali — mitte käsk, vaid paranemis-teekond. Sinu roll: **Õppeosakonna ülem**.
>
> Isiklik link: [PEEGEL_TUUM / vastav tee PDF]
>
> **Valikuline kuulamine (~1h 20 min):** Ava Oma Silmad & Ajuloputus S4 #10 — „Maatriksi Lapsed“  
> Juhi vaatenurgast: kaitse enne tulemust. Kui keegi on ellujäämisrežiimis, ta ei näe plaani (Lisa AT).  
> Kuula: [https://shows.acast.com/avaomasilmad/episodes/ee-jata-navigatsioon-vahele-loomine-avataripilt-ava-oma-silm](https://shows.acast.com/avaomasilmad/episodes/ee-jata-navigatsioon-vahele-loomine-avataripilt-ava-oma-silm) · [katrinlucas.com](https://katrinlucas.com/podcast/) · [Apple EE](https://podcasts.apple.com/ee/podcast/ava-oma-silmad-podcast/id1541890084) · [YouTube](https://www.youtube.com/@AJULOPUTUS)  
> Täpsem kokkuvõte: `kommunikatsioon/soovitus-maatriksi-lapsed.md`
>
> Kui soovid mitte osaleda — üks lause piisab. Austan seda.
>
> Renee Aluste

---

## 8. Sirje Toomla-Õige

| | |
|---|---|
| **E-post** | `sirje.toomla@ramkool.edu.ee` |
| **Organisatsioon** | Rocca al Mare Kool |
| **Roll** | Direktor |
| **Eesmärk** | Kooli kriisivalmidus ja pere tugi |
| **Sidepakk** | P1-C-PERE |
| **Kanal** | E-kiri kool |

### Sinu ülesanded

1. Loe Lisa H (kriis isale) ja Lisa W (Montessori austus)
2. Jaga P1-C-PERE lapsevanematele ainult vabatahtlikult
3. Suuna kriis Eluliinile / koolipsühholoogile
4. Kooskõlasta Helin Vaheriga (lapsevanem)

### Käitumisjuhis

Turvalisus enne loogikat. Ei süüdista vanemaid.

### E-kirja avamine (mustand)

> Tere Sirje,
>
> Jagame sulle Operatsioon „Peegel“ materjali — mitte käsk, vaid paranemis-teekond. Sinu roll: **Direktor**.
>
> Isiklik link: [PEEGEL_TUUM / vastav tee PDF]
>
> **Valikuline kuulamine:** Papsid.ee Podcast  
> Isa / pere rinne — Lisa H, Lisa D, PEEGEL_TEE_C  
> Kuula: [https://papsid.ee/podcast/](https://papsid.ee/podcast/) · [Apple EE](https://podcasts.apple.com/ee/podcast/papsid-ee-podcast/id1768003452)  
> Täpsem: `kommunikatsioon/soovitus-papsid-podcast.md`
>
> Kui soovid mitte osaleda — üks lause piisab. Austan seda.
>
> Renee Aluste

---

## 9. Kristina Šanin

| | |
|---|---|
| **E-post** | `kristina.sanin@waldorf.ee` |
| **Organisatsioon** | Tallinna Vaba Waldorfkool |
| **Roll** | Kooli juhataja |
| **Eesmärk** | Holistiline tugi ja kogukonna sidumine |
| **Sidepakk** | P1-C-PERE |
| **Kanal** | E-kiri |

### Sinu ülesanded

1. Tutvu Ave Osaga (võtmeisik) enne otsust
2. Loe PEEGEL_TEE_C ja Lisa W
3. Anna tagasiside: kas materjal sobib Waldorfi konteksti

### Käitumisjuhis

Austav, aeglane tempo. Laps-keskne keel.

### E-kirja avamine (mustand)

> Tere Kristina,
>
> Jagame sulle Operatsioon „Peegel“ materjali — mitte käsk, vaid paranemis-teekond. Sinu roll: **Kooli juhataja**.
>
> Isiklik link: [PEEGEL_TUUM / vastav tee PDF]
>
> **Valikuline kuulamine:** Papsid.ee Podcast  
> Isa / pere rinne — Lisa H, Lisa D, PEEGEL_TEE_C  
> Kuula: [https://papsid.ee/podcast/](https://papsid.ee/podcast/) · [Apple EE](https://podcasts.apple.com/ee/podcast/papsid-ee-podcast/id1768003452)  
> Täpsem: `kommunikatsioon/soovitus-papsid-podcast.md`
>
> Kui soovid mitte osaleda — üks lause piisab. Austan seda.
>
> Renee Aluste

---

## 10. Ave Osa

| | |
|---|---|
| **E-post** | `tallinn@waldorf.ee` |
| **Organisatsioon** | Tallinna Vaba Waldorfkool |
| **Roll** | Võtmeisik; holistiline tugi |
| **Eesmärk** | Sidestada kogukond ja paranemis-teadlik tugi |
| **Sidepakk** | P1-C-PERE |
| **Kanal** | E-kiri + telefon |

### Sinu ülesanded

1. Loe Lisa AI (Epp Kärsin) ja Lisa W kontekstis
2. Hinda P1-C-PERE sobivust kogukonnale
3. Suuna intiimsuse teemad Epp Kärsinile (Lisa AA)
4. Kontakt telefon: +372 5690 4407

### Käitumisjuhis

Soe, mitte survav. Vabatahtlikkus.

### E-kirja avamine (mustand)

> Tere Ave,
>
> Jagame sulle Operatsioon „Peegel“ materjali — mitte käsk, vaid paranemis-teekond. Sinu roll: **Võtmeisik**.
>
> Isiklik link: [PEEGEL_TUUM / vastav tee PDF]
>
> **Valikuline kuulamine:** Papsid.ee Podcast  
> Isa / pere rinne — Lisa H, Lisa D, PEEGEL_TEE_C  
> Kuula: [https://papsid.ee/podcast/](https://papsid.ee/podcast/) · [Apple EE](https://podcasts.apple.com/ee/podcast/papsid-ee-podcast/id1768003452)  
> Täpsem: `kommunikatsioon/soovitus-papsid-podcast.md`
>
> Kui soovid mitte osaleda — üks lause piisab. Austan seda.
>
> Renee Aluste

---

## 11. Ruth Maria Roosi-Ott

| | |
|---|---|
| **E-post** | `info@mariamontessori.ee` |
| **Organisatsioon** | Montessori võrgustik |
| **Roll** | AMI juhendaja; instituudi juhatus |
| **Eesmärk** | Austus lapse vastu algusest (0–3) |
| **Sidepakk** | P1-C-PERE |
| **Kanal** | E-kiri info@ |

### Sinu ülesanded

1. Loe Lisa W ja Lisa M (identiteet)
2. Jaga P1-C-PERE Montessori õpetajatele vabatahtlikult
3. Anna tagasiside: mis töötab lasteaias

### Käitumisjuhis

Rahulik, austav. Laps ei ole projekt.

### E-kirja avamine (mustand)

> Tere Ruth,
>
> Jagame sulle Operatsioon „Peegel“ materjali — mitte käsk, vaid paranemis-teekond. Sinu roll: **AMI juhendaja**.
>
> Isiklik link: [PEEGEL_TUUM / vastav tee PDF]
>
> **Valikuline kuulamine:** Papsid.ee Podcast  
> Isa / pere rinne — Lisa H, Lisa D, PEEGEL_TEE_C  
> Kuula: [https://papsid.ee/podcast/](https://papsid.ee/podcast/) · [Apple EE](https://podcasts.apple.com/ee/podcast/papsid-ee-podcast/id1768003452)  
> Täpsem: `kommunikatsioon/soovitus-papsid-podcast.md`
>
> Kui soovid mitte osaleda — üks lause piisab. Austan seda.
>
> Renee Aluste

---

## 12. Epp Kärsin

| | |
|---|---|
| **E-post** | `epood@eppkarsin.com` |
| **Organisatsioon** | Amare Luna / teadlik intiimsus |
| **Roll** | Intiimsuse koolitaja |
| **Eesmärk** | Häbist vabanemine; stress kehast välja |
| **Sidepakk** | P1-C-PERE (täiendus) |
| **Kanal** | E-kiri / Instagram @epp.karsin |

### Sinu ülesanded

1. Tutvu Lisa AA ja Lisa AI
2. Suuna huvilised eppkarsin.com — pane end kirja
3. Anna tagasiside: kas Peegel ja sinu töö klapivad

### Käitumisjuhis

Aus, soe. Mitte tabu murdmine ilma turvalisuseta.

### E-kirja avamine (mustand)

> Tere Epp,
>
> Jagame sulle Operatsioon „Peegel“ materjali — mitte käsk, vaid paranemis-teekond. Sinu roll: **Intiimsuse koolitaja**.
>
> Isiklik link: [PEEGEL_TUUM / vastav tee PDF]
>
> **Valikuline kuulamine:** Papsid.ee Podcast  
> Isa / pere rinne — Lisa H, Lisa D, PEEGEL_TEE_C  
> Kuula: [https://papsid.ee/podcast/](https://papsid.ee/podcast/) · [Apple EE](https://podcasts.apple.com/ee/podcast/papsid-ee-podcast/id1768003452)  
> Täpsem: `kommunikatsioon/soovitus-papsid-podcast.md`
>
> Kui soovid mitte osaleda — üks lause piisab. Austan seda.
>
> Renee Aluste

---

## 13. Jelena Pribylski

| | |
|---|---|
| **E-post** | `jelena.pribylski@pk.ee` |
| **Organisatsioon** | PK (pereteraapia) |
| **Roll** | Koordinaator |
| **Eesmärk** | Pereteraapia võrgustiku sidumine |
| **Sidepakk** | P1-A-KRIIS (suunamine) |
| **Kanal** | E-kiri |

### Sinu ülesanded

1. Tutvu Lisa P ja Lisa H (kriis)
2. Hinda suunamise võimalust peredele
3. Trauma-teadlik suunamine — mitte diagnoosimine

### Käitumisjuhis

Professionaalne konfidentsiaalsus. Valideerimine.

### E-kirja avamine (mustand)

> Tere Jelena,
>
> Jagame sulle Operatsioon „Peegel“ materjali — mitte käsk, vaid paranemis-teekond. Sinu roll: **Koordinaator**.
>
> Isiklik link: [PEEGEL_TUUM / vastav tee PDF]
>
> **Valikuline kuulamine:** Papsid.ee Podcast  
> Isa kriisis — Lisa H kontekstis  
> Kuula: [https://papsid.ee/podcast/](https://papsid.ee/podcast/) · [Apple EE](https://podcasts.apple.com/ee/podcast/papsid-ee-podcast/id1768003452)  
> Täpsem: `kommunikatsioon/soovitus-papsid-podcast.md`
>
> Kui soovid mitte osaleda — üks lause piisab. Austan seda.
>
> Renee Aluste

---

## 14. Tiit Trofimov

| | |
|---|---|
| **E-post** | `info@tiittrofimov.ee` |
| **Organisatsioon** | Eneseareng |
| **Roll** | Emotsioonide ja teadlikkuse töö |
| **Eesmärk** | Meeste ringid; taastumine pärast põhja |
| **Sidepakk** | P1-A-KRIIS |
| **Kanal** | E-kiri / tel +372 5626 2454 |

### Sinu ülesanded

1. Loe Lisa H ja raamat F (murra ring)
2. Suuna mehi, kes on põhjas, PEEGEL_TEE_A-sse
3. Anna tagasiside: mis töötab meeste ringis

### Käitumisjuhis

Aus, mitte üleolev. Keha enne mõistust.

### E-kirja avamine (mustand)

> Tere Tiit,
>
> Jagame sulle Operatsioon „Peegel“ materjali — mitte käsk, vaid paranemis-teekond. Sinu roll: **Emotsioonide ja teadlikkuse töö**.
>
> Isiklik link: [PEEGEL_TUUM / vastav tee PDF]
>
> **Valikuline kuulamine:** Papsid.ee Podcast  
> Isa kriisis — Lisa H kontekstis  
> Kuula: [https://papsid.ee/podcast/](https://papsid.ee/podcast/) · [Apple EE](https://podcasts.apple.com/ee/podcast/papsid-ee-podcast/id1768003452)  
> Täpsem: `kommunikatsioon/soovitus-papsid-podcast.md`
>
> Kui soovid mitte osaleda — üks lause piisab. Austan seda.
>
> Renee Aluste

---

## 15. Ülo Vooglaid

| | |
|---|---|
| **E-post** | `ylo@vooglaid.org` |
| **Organisatsioon** | Sihtasutus Ülo Vooglaiu mõttepärand |
| **Roll** | Kodaniku-mõtleja eeskuju; demonstraator |
| **Eesmärk** | Näidata, et elanikust kodanikuks on Eestis juba kirjas |
| **Sidepakk** | P1-B-SKEPTIK (vooglaid tuum) |
| **Kanal** | E-kiri isiklik / sihtasutus@vooglaid.org |

### Sinu ülesanded

1. Soovita *Elanikust kodanikuks* enne OPORD-i (tee B)
2. Anna tagasiside: kas Peegel ja Vooglaid klapivad
3. Jaga ainult vabatahtlikult — mitte surve all

### Käitumisjuhis

Rahulik, kriitiline. Austa autonoomiat. Lisa P teiste suhtes.

### E-kirja avamine (mustand)

> Tere Ülo,
>
> Jagame sulle Operatsioon „Peegel“ materjali — mitte käsk, vaid paranemis-teekond. Sinu roll: **Kodaniku-mõtleja eeskuju**.
>
> Isiklik link: [PEEGEL_TUUM / vastav tee PDF]
>
> **Valikuline kuulamine (~1h 20 min):** Ava Oma Silmad & Ajuloputus S4 #10 — „Maatriksi Lapsed“  
> Valikuline teadlikkuse kanal — sobib neile, kes tahavad sügavamat konteksti enne OPORD-i.  
> Kuula: [https://shows.acast.com/avaomasilmad/episodes/ee-jata-navigatsioon-vahele-loomine-avataripilt-ava-oma-silm](https://shows.acast.com/avaomasilmad/episodes/ee-jata-navigatsioon-vahele-loomine-avataripilt-ava-oma-silm) · [katrinlucas.com](https://katrinlucas.com/podcast/) · [Apple EE](https://podcasts.apple.com/ee/podcast/ava-oma-silmad-podcast/id1541890084) · [YouTube](https://www.youtube.com/@AJULOPUTUS)  
> Täpsem kokkuvõte: `kommunikatsioon/soovitus-maatriksi-lapsed.md`
>
> Kui soovid mitte osaleda — üks lause piisab. Austan seda.
>
> Renee Aluste

---

## 16. Indrek Paal

| | |
|---|---|
| **E-post** | `sihtasutus@vooglaid.org` |
| **Organisatsioon** | Ülo Vooglaiu Kirjastus / Vana-Viru Kaubaveod |
| **Roll** | Praktiline demonstraator; mõttepäranda hoidja |
| **Eesmärk** | Tuua Vooglaidi tuum kätte — raamat, vestlus |
| **Sidepakk** | P0-TUUM + Lisa AR |
| **Kanal** | E-kiri / tel 667 0111 |

### Sinu ülesanded

1. Levita *Vanaisa uued lood* (2025) sobivates ringkondades
2. Siduda sihtasutus ja Peegel ainult Vooglaidi nõusolekul
3. Anna tagasiside: mis formaat töötab

### Käitumisjuhis

Asjatundlik, praktiline. Lisa Q austav keel. Mitte masspostitus.

### E-kirja avamine (mustand)

> Tere Indrek,
>
> Jagame sulle Operatsioon „Peegel“ materjali — mitte käsk, vaid paranemis-teekond. Sinu roll: **Praktiline demonstraator**.
>
> Isiklik link: [PEEGEL_TUUM / vastav tee PDF]
>
> **Valikuline kuulamine (~1h 20 min):** Ava Oma Silmad & Ajuloputus S4 #10 — „Maatriksi Lapsed“  
> Valikuline teadlikkuse kanal — sobib neile, kes tahavad sügavamat konteksti enne OPORD-i.  
> Kuula: [https://shows.acast.com/avaomasilmad/episodes/ee-jata-navigatsioon-vahele-loomine-avataripilt-ava-oma-silm](https://shows.acast.com/avaomasilmad/episodes/ee-jata-navigatsioon-vahele-loomine-avataripilt-ava-oma-silm) · [katrinlucas.com](https://katrinlucas.com/podcast/) · [Apple EE](https://podcasts.apple.com/ee/podcast/ava-oma-silmad-podcast/id1541890084) · [YouTube](https://www.youtube.com/@AJULOPUTUS)  
> Täpsem kokkuvõte: `kommunikatsioon/soovitus-maatriksi-lapsed.md`
>
> Kui soovid mitte osaleda — üks lause piisab. Austan seda.
>
> Renee Aluste

---

## Osalejad ilma e-postita (repos)

| Nimi | Roll | Kuidas ühenduda |
|------|------|----------------|
| Riho Ühtegi | Strateegiline juht | Avalik kanal |
| Rene Toomse | Riigi tase | Avalik kanal |
| Eerik Heldna | Kriisireguleerimine | PPA |
| Elmar Vaher | RKIK | kaitseinvesteeringud.ee |
| Heli Illipe-Sootak | Steiger peakirjastaja | Messenger / otsing |
| Mihhail Ušakov | RU peer-educator (kandidaat) | Isiklik kohtumine |
| Ain Anslan | Viru vangla | Vanglateenistus |
| Aigar Ojaots | Pertinax | MTÜ reg 80634291 |
| Peeter Järvsoo | Noorte Kotkad | nooredkotkad.ee |
| Andrei Ambros | Harku judo | estjutsu.ee |
| Eero Kinnunen | Veteranid | ekvv.ee |
| Margus Lõoke | KV mustri tunnistaja | Isiklik kontakt Renee |

---

*Lisa: kommunikatsioon/osalejate-kohandatud-kask.md*
