# Lisa AG — Rollid, juhised ja NL-skeem (aus partnerlus)

**Operatsioon „Peegel"** | Pere struktuur | Põhimõte: *tunnustame üldises plaanis käitumisi, mis toovad tagasi **mehe rolli ja ülesanded** — ja **naine saab samuti juhised**. See meenutab **Nõukogude Liidu pere-skeemi**. Võib-olla on see **hetkel parim lahendus**.*

**Kohustuslik (meestele ja partnerlusele):** Orion Taraban — *The Value of Others* (2024, PsycHacks, ISBN 979-8990069510) — kapten/reisija, realistlik suhtemudel.

**Seotud:** Lisa AC (ownership, magamistuba), Lisa Z (tööstus vs partnerlus), Lisa E (pere rindejoon), Lisa AA (intiimsus), Lisa AE (olukorra teadlikkus), Lisa W (austus), Lisa I (Steiger), Lisa P (trauma)

> *🫡 **Ära usu mind. Kaitse eelkõige oma uskumusi ja veendumusi.** Uuri. Kaalu. Vali ise.*

> *See ei ole poliitiline programm. See ei ole nostalgia stalinismi järele. See on **aus sõnastus**: kui lääne individualism ja tööstuslik eraldatus on pere lõhkunud — **rollide selgus** (nagu NL ajal tuntud) võib olla parim, mis meil praegu on.*

---

## Renee hüpotees — selgelt

| Mis see ON | Mis see EI OLE |
|------------|----------------|
| **Tunnustamine** — mehe ja naise rollid selged, vastastikused | Riigi käsk ellu minna nagu 1980 |
| **Juhised mõlemale** — mees ja naine saavad oma checklisti | „Naine vaikib, mees käsib" |
| **NL-skeem** ausalt nimetatud — rollid, mitte partei | NL-aegne represseerimine kui eesmärk |
| **Pragmaatiline** — võib-olla parim lahendus **praegu** | „Ainus tõde igavesti" |
| Käitumised, mis **toovad tagasi** isa kohaloleku, vastutuse, suuna | Naiste või meeste alavääristamine |

**Üks lause:**

> *Kui keegi ei tea, kes vastutab — kõik on õnnetud. NL-skeem andis rollid (valesti riigiga seotult). Me võtame **selguse** — ilma riigi sundita — ja tunnustame käitumisi, mis taastavad mehe rolli ja annavad naisele **samuti** juhised.*

---

## 1. Mis on „NL-skeem" siin — ja miks seda nimetada

### Mida NL pere-mudel sisuliselt tähendas

| Element | NL kontekst | Meie kontekst (Operatsioon) |
|---------|-------------|------------------------------|
| **Mees** | Töötaja, toitja, „pea pere" | **Vastutav**, kohal, suunda näitav — mitte tiran |
| **Naine** | Kodu, lapsed, „taust" | **Partner** oma rolliga — mitte vaikiv ornament |
| **Lapsed** | Riigi ja pere tulevik | **Rindejoon** (Lisa E) — austus algusest (Lisa W) |
| **Riik** | Kõik teadis „õiget" mudelit | **Ei** — pere otsustab ise, kodanikualgatus |
| **Häbi** | Vale elu ei räägitud | **Ausus** — trauma, abi, Epp, Lisa H |

### Miks see võib olla **hetkel parim lahendus**

| Probleem täna (Lisa Z) | NL-skeemi „kingitus" (ilma NL-i kurjuseta) |
|------------------------|---------------------------------------------|
| Isa eemal, ema ülekoormatud | **Selge**: kes viib raha, kes hoiab kodu |
| Kõik on „vabad", keegi ei vastuta | **Rollid** — mitte orjus, vaid **leping** |
| Mehed emasestunud / hüplevad | **Mehe roll** tagasi — ownership (Lisa AC) |
| Naised üksi karbis | **Naise juhised** — mitte üksildus |
| Lapsed ilma eeskujuta | Isa **kohal** — vähemalt eesmärk |

> *Võib-olla on see NL-skeem. Võib-olla on see ainus, mida meie ühiskond **oskab** praegu — sest NL jäljed on Eestis alles. Parem aus skeem kui vale lääne unistus, mis pere lõhub.*

**Mis me EI võta NL-ist:** represseerimine, partei, sundabielu, alkoholismi varjamine, naise kui second-class kodanik.

---

## 2. Mida me **tunnustame** — mehe roll ja käitumised

*Üldises plaanis. Iga pere lepib detailid kokku. See ei ole käsk.*

### Mehe roll — tuum

| Roll | Tähendus praktikas |
|------|-------------------|
| **Toitja / teenija** | Vii raha ja turvalisus koju — töö, ettevõtlus (Lisa Z) |
| **Suuna näitaja** | Näita teed — Intiimsuse atlas, Epp, uni, liikumine (Lisa AC, AA) |
| **Ownership** | „See on minu vastutus" — mitte süüdistus naist |
| **Kaitseja** | Cooper — ole teadlik; kaitse peret, ära domineeri (Lisa P) |
| **Isa** | Eeskuju lastele — kohal, mitte ainult nädalavahetusel (Lisa H, E) |
| **Partner** | Üks laev — mitte võistlus magamistoas (Lisa AC, Z) |

### Käitumised, mida **tunnustame** (mees)

| # | Käitumine | Miks |
|---|-----------|------|
| 1 | **Töötab** ja toob koju — ausalt, ilma valetamiseta | Toitja roll |
| 2 | **Õpib** — Intiimsuse atlas, Lisa AF, uni, toit | Näitab teed |
| 3 | **Võtab vastutuse** — ei süüista naist ega lapsi | Ownership |
| 4 | **On kohal** — füüsiliselt ja emotsionaalselt | Vastu tööstusmudelile (Lisa Z) |
| 5 | **Räägib ausalt** — ka ebamugavust | Vastu NL varjamisele |
| 6 | **Kaitseb** ilma omamiseta — AE blokk 0 | Partner, mitte vangla ülem |
| 7 | **Palub abi** — narko.ee, terapeut, Lisa H | Tugevus, mitte häbi |
| 8 | **Juhib pere koosolekut** — kord nädalas „perede aeg" | Struktuur |
| 9 | **Ei jookse** — konfliktist, intiimsusest, vastutusest | Mehe roll püsib |
| 10 | **Näitab lastele** — kuidas mees käitub | Rindejoon |

> *Need käitumised **toovad tagasi mehe rolli** — mitte diktatuuri, vaid **usaldusväärse isa ja partneri**.*

---

## 2b. Meeste SOP — igapäevased näited (konkreetselt)

*Renee Aluste isiklik standard — mitte teooria. Lisa N demomehed võivad võtta ühe rea korraga.*

### Renee tunnistus

> *Mina avan **alati** oma naisele ukse — **sisse ja välja** minnes. Tassin **kogu kraami**, mis on võimalik. See ei ole teatrietendus. See on **meeste SOP** — väike käitumine, mis ütleb: *ma olen kohal, ma teen, ma juhin hellalt*. *

### Meeste SOP — käitumiste nimekiri

| # | Käitumine | Mida see õpetab |
|---|-----------|-----------------|
| 1 | **Ava uks** — sisse ja välja, alati (kui koos) | Austus, juhtimine ilma kõrge hääleta |
| 2 | **Tassi kõik** — kotid, pakid, mis tahes | Teenija-roll — toitja ei ole ainult palk |
| 3 | **Vii naise kohtingule** — planeeri, kutsu, vii välja | Kapten võtab reisija pardale (vt §2c) |
| 4 | **Maksa / korralda** — kui kokku lepitud | Selgus, mitte võistlus arvega |
| 5 | **Ole esimene, kes astub sisse** — ja viimane, kes kontrollib turvalisust | Kaitseja |
| 6 | **Küsi päeva kohta** — kuula enne nõu | Partner, mitte monoloog |
| 7 | **Aita mantliga** — auto, trepp, laps kärus | Kohalolek |
| 8 | **Täida lubadus** — väike asi täna | Usaldus (Lisa I) |
| 9 | **Perede aeg** — kutsu ise kokku | Kapten kutsub laevale |
| 10 | **Kui eksid — paranda** — üks lause, mitte kaitsepositsioon | Ownership (Lisa AC) |

### Minimaalne SOP (taskus)

```
UKS · KOTID · KOHTING · KUULA · KOHAL
```

> *Need ei asenda Tarabani raamatut ega Intiimsuse atlasi. Need on **harjutus** — nagu 90 sek rippumist (Lisa AB).*

---

## 2c. Kapten ja laev — Orion Taraban (*The Value of Others*)

> *🫡 **Ära usu mind.** Loe Taraban ise. See on **realistlik** mudel — mitte romantiline muinasjutt.*

### Kohustuslik kirjandus

**Orion Taraban** — *The Value of Others: Understanding the Economic Model of Relationships* (PsycHacks, 2024, ~432 lk, ISBN 979-8990069510). Kliiniline psühholoog; PsycHacks podcast.

| Miks OPORD-is | Mida see annab |
|---------------|----------------|
| **Realism** — mitte „kõik läheb hästi" | Suhted kui **väärtuse** ja **valiku** ruum |
| Kapten / reisija metafoor | Mehe ja naise rollid **selgelt** |
| Aus ökonoomika | Miks partner jääb või lahkub — ilma moraalmonoloogita |

### Metafoor — dokis

Taraban kirjeldab **seksuaalset turgu** kui dokki:

| Roll | Kes | Mida teeb |
|------|-----|-----------|
| **Kapten** | Üldjuhul mees / initsiaator | Pakub reisi — laev, suund, juhtimine |
| **Reisija** | Üldjuhul naine / valija | Otsustab, kas tulla pardale; hindab laeva |

**Operatsiooni tõlge peresse (mitte pickup):**

> *Mees **demonstrerib juhtimisomadusi kaptenina** ja **võtab reisija pardale** — viib naise **kohtingule**, näitab suunda, hoolitseb laeva eest.*

### Kui laev liigub — pole probleemi

| Olukord | Tähendus |
|---------|----------|
| **Kokkulepitud suund** | Pere eesmärk on selge — raha, lapsed, kodu, kasv |
| **Mees juhib rooli** | Ta teeb SOP-i — uks, kotid, kohal, vastutus |
| **Reisija usaldab** | Naine ei pea rooli võtma — ta **valis** selle kapteni |

> *Kui laev liigub kokkulepitud suunas ja mees juhib — **polegi probleemi**.*

### Kui torm tuleb — kapten keerab tagasi

| Olukord | Kapteni tegevus |
|---------|-----------------|
| **Raskused** — tervis, raha, trauma, lapsepuhkus | Laev **kaldub** — see on normaalne |
| **Kapten leiab uue asimuudi** | Õpib (Lisa AA, AF, H), palub abi, korrigeerib |
| **Rool tagasi sihtpunkti poole** | Ownership — mitte süüdistus reisijat |

> *Kaptenid **karastatakse tormisel merel** pidevalt. Kogenematus ei ole põhjus rooli jätta — see on põhjus **õppida**.*

### Mässamine — millal läheb valesti

| Mässamine | Mis juhtub |
|-----------|------------|
| **Keegi ei keera rooli tagasi** | Mees jookseb, joob, hüpleb, kaob |
| **Laev liigub karile** | Pere laguneb, lapsed kannatavad, usaldus puruneb |
| **Naine võtab rooli üle** | Ta **näeb** karpi — ja võtab üle, sest **kapten ei juhi** |

**See ei tee naist kurjaks.** See teeb ta **reisijaks, kes hakkas kapteniks**, sest laev oli ohus. Lisa AC + Lisa P.

> *Probleem ei ole „naine võttis üle". Probleem on: **rooli ei keeratud tagasi** õigel ajal.*

### Sihtkoht — naise valik

Kui **suund on paigas** ja **oskus kohaneda** teekonnal on olemas:

| Tulemus | Mida see tähendab |
|---------|-------------------|
| **Sihtkoht** | Pere jõuab — turvalisus, kasv, järgmine etapp |
| **Naise otsus** | Kas ta liigub edasi **selle kapteniga** järgmisse ettevõtmisse / eluetappi |
| **Või lahkub** | Suhe lõpeb — tuleb **järgmine reisija** (Taraban: turg on reaalne) |

> *Me oleme **realistid**. Mitte iga kapten ja reisija ei sõida igavesti. Aga kapten, kes **juhib**, **parandab** ja **teenib** usaldust — tõenäolisemalt jõuab koos järgmise reisini.*

### Taraban + NL-skeem + Operatsioon

| Taraban | OPORD |
|---------|-------|
| Väärtus ja valik | Lisa AG tunnustamine |
| Kapten peab laeva väärt | Mehe SOP — uks, kotid, kohal |
| Reisija valib | Naise juhised — mitte sund |
| Maintenance ei lõpe | Perede aeg, Lisa E |
| Aus ökonoomika | „Ära usu mind" — loe ise |

---

## 3. Naise **juhised** — samuti selged

*Naised ei ole „ilma juhisteta". Nad said NL ajal **oma** rolli — me anname **teadlikud** juhised, mitte ühiskonna müra.*

### Naise roll — tuum (partner, mitte teenija)

| Roll | Tähendus praktikas |
|------|-------------------|
| **Kodu ja laste ankur** | Kui nii on kokku lepitud — **au sees**, mitte „vähem tähtis" |
| **Partner** | Otsustab koos — mitte ainult täidab |
| **Keha ja intiimsus** | Epp, yoni, *to heal is to feel* (Lisa AA) |
| **Usaldus** | Luba mehel olla mees — AE blokk 0: ära blokeeri |
| **Eeskuju** | Lapsele ema — austus, mitte drama (Lisa W) |

### Juhised naisele (checklist)

| # | Juhend | Lisa |
|---|--------|------|
| 1 | **Loe** *Intiimsuse atlasi* — mõista oma keha | AA |
| 2 | **Pane end kirja** Epp Kärsinile — yoni, ausus | AA |
| 3 | **Ära blokeeri** meest „minu mees" omamisega | AE, AC |
| 4 | **Räägi** — mida vajad; ära oota loetlemist | P, Q |
| 5 | **Toeta** mehe vastutust — ära alanda ega kontrolli GPS-iga | AC |
| 6 | **Hoolitse endast** — uni, liikumine, õhk (Lisa AE) | AE, V |
| 7 | **Trauma** — kui triggeris, ütle; ära lase kehal laskma (Lisa P) | P |
| 8 | **Lapsed** — austus algusest; Montessori beebi (Lisa W) | W |
| 9 | **Ära võrdle** meest teistega — sotsiaalmeedia | Z |
| 10 | **Partnerlus** — üks laev; ettevõtlus koos võimalusel | Z |

> *Naine saab **juhised** — sama rangusega kui mees. See ei ole alandamine. See on **selgus**.*

---

## 4. Ühine leping — NL-skeem ilma riigita

### Pere „NL-skeem" tänapäevas (vabatahtlik)

```
MEES: toitja + suund + kohal + aus + kaitseb ilma omamiseta
NAINE: partner + kodu/lapsed (kui lepitud) + oma keha + usaldus + juhised täidetud
KOOS: perede aeg · üks laev · lapsed rindejoon · abi kui vaja
RIIK: EI otsusta teie rollide üle — teie ise
```

### Tunnustamise tabel (üldine plaan)

| Kui mees teeb… | Tunnustame |
|----------------|------------|
| Töötab ja toob koju | Jah — toitja roll |
| Õpib ja näitab teed | Jah — juht rollis |
| Võtab vastutuse | Jah — ownership |
| Palub abi | Jah — tugevus |
| On kohal lastega | Jah — isa |

| Kui naine teeb… | Tunnustame |
|-----------------|------------|
| Toetab mehe vastutust ilma alandamiseta | Jah — partner |
| Hoolitseb endast ja kehast | Jah — Epp, AA |
| Ei blokeeri omamisega | Jah — AE blokk 0 |
| Räägib ausalt | Jah — P, Q |
| Kasvatab last austusega | Jah — W |

---

## 6. Mis läheb valesti — kui skeem muutub sunduseks

| NL viga | Meie vastus |
|---------|-------------|
| Riik otsustab | **Pere** otsustab |
| Mees peab jooma | **Ei** — Lisa H, abi |
| Naine ei tohi lahkuda | **Ei** — ohvriabi 116 006 |
| Rollid ilma armastuseta | **Ei** — Lisa E, intiimsus |
| Häbi ja varjamine | **Ei** — ausus, Lisa R |

> *Skeem on **tööriist**, mitte vangla. Kui skeem teeb haiget — muuda skeemi. Abi on olemas.*

---

## 7. Kuidas see seondub teiste lisadega

| Lisa | Roll |
|------|------|
| **AC** | Mehe revolutsioon, ownership, magamistuba |
| **Z** | Miks tööstusmudel ebaõnnestus — partnerlus asemel |
| **E** | Pere rindejoon |
| **AA** | Mõlemale — keha, intiimsus |
| **AE** | Olukorra teadlikkus, omamise blokk |
| **AF** | Mehe keha — erektsioon, relva hooldus |
| **I** | Juhtimine — V-formatsioon, keegi ei veda igavesti |
| **P** | Trauma — rollid ei kehti triggeris |
| **Taraban** | *The Value of Others* — kapten/reisija, realism |

---

## 8. AVATUD MEEL — küsimused mõlemale

1. Kas meie peres on **rollid** selged — või kõik ootavad, et teine arvab?
2. Kas me **tunnustame** mehe toitja/isa rolli — või alandame?
3. Kas naine on saanud **juhised** — või ainult müra sotsiaalmeediast?
4. Kas see on **NL-skeem** — ja kas me oleme ausad sellega?
5. Kas see on **hetkel parim** — või pelgame seda tunnistada?
6. Kas **riik** või **sund** juhib meid — või meie ise?
7. Kas ma juhin **kaptenina** — või ootan, et naine võtaks rooli?
8. Kas ma olen lugenud **Tarabanit** — või elan illusioonis?

---

## 9. Üks lause kogu Lisa AG kohta

> *Ava uks. Tassi kotid. Juhi nagu kapten. Loe Tarabanit — ole realist. Tunnusta mehe rolli, anna naisele juhised. NL-skeem võib olla parim, mis meil praegu on. Kui rool keeratakse tagasi sihtpoole — laev jõuab. Kui mitte — reisija võtab üle või lahkub. Aus partnerlus.*

---

*Lisa AG — Operatsioon „Peegel". Rollid ja juhised — vabatahtlik pere leping. Vägivald: **112**, ohvriabi **116 006**, Eluliin **655 8088**.*
