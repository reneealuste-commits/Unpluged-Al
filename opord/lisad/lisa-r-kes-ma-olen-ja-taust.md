# Lisa R — Kes ma olen? Taust ja tõendid

> **Lugeja saab:** koordinaatori tausta ja usaldusväärsuse fakte.  
> **Loe seda kui:** kahtled, kes see inimene on (tee B).  
> **Ära loe kui:** usaldus on juba olemas — vali oma tee.


**Operatsioon „Peegel"** | Kui keegi kahtleb või küsib: *kes sa oled ja mis on su taust?*  
**Allikas:** Renee Aluste ametlik CV (kutse taotleja, täiskasvanute koolitaja tase 8) — Google Docs, 2026  
**Täiendab:** [renee-aluste-profiil.md](renee-aluste-profiil.md), OPORD Q&A II

> *Ma ei palu sul uskuda sõnu. Ma palun sul kontrollida fakte.*

---

## 1. Üks lause vastus

**Renee Aluste** on Eesti kaitseväe ja Kaitseliidu taustaga juhtimisinstruktor, USA Army Ranger Schooli läbinud eriväelane, **endine**

---

## 2. Kui keegi kahtleb — mida öelda

| Samm | Tee |
|------|-----|
| **1. Valideeri** | „Loogiline küsida, kes ma olen ja miks sa peaksid hoolima." (Lisa P) |
| **2. Anna faktid** | Lühike taust allpool — haridus, teenistus, koolitused |
| **4. Ära müü** | „Kontrolli ise. Kui see ei klapi — lahku rahus." |
| **5. Suuna** | Täielik isiklik lugu: **Lisa H**, **renee-aluste-profiil.md** |

> *Kahtlus ei ole vaen. See on terve reaktsioon infosõjas. Me vastame aususega — mitte survega.*

---

## 3. Põhiandmed

| | |
|---|---|
| **Nimi** | Renee Aluste |
| **Elukoht** | Tallinn |
| **Emakeel** | eesti keel |
| **Inglise keel** | C1 (mõistmine, rääkimine, kirjutamine) |
| **Avalik roll** | Operatsioon „Peegel" koordinaator; Kaitseliidu instruktor |

---

## 4. Haridus (kontrollitav)

| Aasta | Asutus | Tunnistus / tase |
|-------|--------|------------------|
| 2014 | Kaitseväe Ühendatud Õppeasutused — sõjaväeline juhtimine, tase 5 | Lõputunnistus A030233 — *„on täitnud kutseõppe õppekava sõjaväelise juhtimise eriala vanemallohvitseridele täies mahus"* |
| 2020 | Luua Metsanduskool — arborist, tase 4 | Lõputunnistus K017839 |
| 2009 | Pärnumaa Kutsehariduskeskus — metsamajandus | Lõputunnistus P029205 |

---

## 5. Teenistuskäik ja kutse

| Periood | Organisatsioon | Roll |
|---------|----------------|------|
| **2008–2018** | Eesti Kaitsevägi | Erioperatsioonide grupi (EOG) operaator, sniprigrupi ülem, jaoülem. Taktikaline väljaõpe, relvaõpe, meeskonnajuhtimine, operatsioonide planeerimine |
| **2009–2010** | Vahipataljon (ajateenistus) | — |
| **2015** | Estpla-21, UNIFIL Liibanon | ~7 kuud Soome-Iiri pataljoni koosseisus |
| **2018–…** | Kaitseliit, Harju malev | Kompaniiülem ja instruktor. Sõdurioskuste kursuse (SOK) planeerimine, läbiviimine ja hindamine |
| **2022** | USA Army Ranger School | Klass 07-22 — juhtimine surve all, meeskonnatöö, vastupidavus |

---

## 6. Koolituskogemus (kokkuvõte)

### Kaitseliit — Sõdurioskuste kursus (SOK)

| Aasta | Korraldaja | Maht |
|-------|------------|------|
| 2020–2025 | Kaitseliit Harju malev (Männiku tee 121, Tallinn) | **6× täiskursus**, igaüks **240+ akadeemilist tundi** (+ ettevalmistus ja järeltöö) |

**Kontakt kinnitamiseks:** harju@kaitseliit.ee · +372 717 9989


Juhtimisinstruktor / rollimängija / abi-instruktor **50+** üritusel, sh:

| Klient / üritus | Tüüp |
|-----------------|------|
| Riigikohus, Coop Pank, LHV, Telia, Bolt, NetGroup, Merit, Milrem, Veriff | FTX / JPL / juhtimistreening |
| EAS, Junior Achievement, Ideal, Viking Sec, Adduco | Lahingmatk / meeskond |
| Helifors, LTKH (haigla) | Keynote + tegevus |

**Kontakt:** info@reneealuste.com · +372 59055288 · [reneealuste.com)

### Muu panus täiskasvanute koolituses

- **Imelised aastad** — lastevanemate koolitusprogramm (osalemine ja rakendamine)
- Koolitusmaterjalid: stsenaariumid, ülesanded, refleksioonikaardid, ohutusjuhendid
- Avalikud esinemised ja kogukonna kaasamine

---

## 7. Avalikud tõendid — meedia ja publikatsioonid

Need on **sõltumatult kontrollitavad** — mitte operatsiooni enda väited.

| Tüüp | Pealkiri | Link |
|------|----------|------|
| Raamat | *Ranger Kool 07-22: Isiklik kogemuslugu USA Ranger Schoolist* (isekirjastatud, ~99 lk) | [Google Drive PDF](https://drive.google.com/file/d/1tADnTfH9qsh_81_2poPG3R-aQ4RFq4pu/view?usp=drive_link) |
| Intervjuu | *Eriväelase jutud: Rangerite koolis tahetakse, et nende peale karjutaks* (Delfi Forte, 5.10.2024) | [delfi.ee](https://forte.delfi.ee/artikkel/120326746/erivaelase-jutud-renee-aluste-rangerite-koolis-tahetakse-et-nende-peale-karjutaks-see-on-ainuke-asi-mis-seal-tootab) |
| Artikkel | *Ranger, kes oskab langetada ka suuri ja ohtlikke puid* (Kaitse Kodu 3/2023) | [issuu.com](https://issuu.com/kaitse_kodu/docs/kaitse_kodu_03_2023_veeb/s/20777210) |
| Keynote | *Kuidas juhtida iseennast, et inspireerida teisi* (Personaliuudised) | [YouTube](https://www.youtube.com/watch?v=FGbBS2Pcbng) |
| Podcast | *Tänulikkuse podcast* #22 (16.02.2024) | [tanulikkus.ee](https://www.tanulikkus.ee/et/renee-aluste) |
| Uudis | *Ajukasvajaga võidelnud eestlane läbis USAs karmi väljaõppe* (Postimees, 17.09.2022) | [postimees.ee](https://www.postimees.ee/7607020/taust-ajukasvajaga-voidelnud-eestlane-labis-usas-karmi-valjaoppe) |

---

## 8. Mida see tõestab — ja mida mitte

| ✅ Tõestab | ❌ Ei tõesta |
|-----------|-------------|
| Sõjaline ja Kaitseliidu väljaõpe | Et Operatsioon „Peegel" on riiklik käsk |
| Juhtimiskoolitaja kogemus ettevõtetes ja Kaitseliidus | Et kõik operatsiooni väited on tõed |
| Avalik meedia ja sõltumatud intervjuud | Et sa pead uskuma ilma kontrollimata |
| Isiklik lugu (ajukasvaja, Ranger, depressioon) on dokumenteeritud | Et Renee on eksimatu |

**Operatsioon „Peegel"** on **kodanikualgatus**. Renee taust annab **pädevuse** juhtimisele, koolitusele ja distsipliinile — mitte **volitust** sinu eest otsustada.

---

## 9. Ideaalne sooritus — vestlus kahtlejaga

| Olukord | ❌ Vale | ✅ Ideaalne sooritus |
|---------|--------|----------------------|
| „Sa oled vandenõuteooria müüja" | Solvumine | „Kontrolli allikaid. Kui ei klapi — lahku. Ma ei varja kavatsust." |
| „Miks ma peaksin hoolima?" | Monoloog | „Sa ei pea. Küsimus on: kas see, mida sa iga päev teed, ehitab sind üles või lõhub?" |

---

## 10. Kuhu edasi

| Vajadus | Vaata |
|---------|-------|
| Isiklik lugu, depressioon, taastumine | **Lisa H**, **renee-aluste-profiil.md** |
| Operatsiooni tuum (60 sek) | OPORD — **MIKS? AJEND. EESMÄRK.** |
| Skeptik / WTF / valideerimine | **Lisa P** |
| Täielik Q&A | OPORD sektsioon IV |

---

*Renee Aluste — taust ja tõendid. Avalik. Kontrolli ise.*
